...
def _init_graph(
    ...
    for i in range(len(initial_values)):
        line, = ax.plot([initial_steps[i]], [initial_values[i]], 
...
def _update_graph(name, new_values, new_steps):
    ...
    for i, (line, x_list, y_list) in enumerate(zip(graph['lines'], graph['x_data'], graph['y_data'])):
        if i >= min_length:
            break
...
def plot_rt_graph(
    ...
    if name not in _GRAPHS:
        _init_graph(name, value, step, x_label=x_label, y_label=y_label, 
                   linestyles=linestyle, markers=marker, colors=color)
    else:
