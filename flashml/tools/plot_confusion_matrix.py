...
def _compute_confusion_matrix(pred, targ, labels):
    ...
    cm = np.bincount(indices.astype(np.int64), minlength=n_labels * n_labels).reshape(
        n_labels, n_labels
    )
...
def plot_confusion_matrix(
    ...
    if classes is None:
        ...
        assert len(classes) == len(unique_labels), (
            f"Classes list length ({len(classes)}) doesn't equal the total labels found ({len(unique_labels)})."
        )
    ...
    assert len(unique_labels) < 129, (
        f"Too many classes, your pc will blow up {len(unique_labels)}"
    )
