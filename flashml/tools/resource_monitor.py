...
last_net_io = None; last_disk_io = None; last_time = None
nvml_handles = []; nvml_initialized = False; _pynvml_imported = False; pynvml = None 
...
def is_monitor_running():
    if os.path.exists(LOCKFILE):
        try:
            with open(LOCKFILE, "r") as f: pid = int(f.read().strip())
            ...
            print("Removing stale lock file..."); remove_lockfile()
        except (ValueError, FileNotFoundError, EnvironmentError) as e:
            print(f"Error checking lock file, removing it: {e}"); remove_lockfile()
        except Exception as e: print(f"Unexpected error checking lock file: {e}"); pass # Ignore other errors like permission denied
    return False

def write_lockfile():
    try:
        with open(LOCKFILE, "w") as f: f.write(str(os.getpid()))
    except EnvironmentError as e: print(f"Error writing lock file: {e}")
        ...
        if os.path.exists(LOCKFILE):
            try:
                 ...
                 try:
                      ...
                      print(f"Warning: Could not remove potentially corrupt lock file {LOCKFILE}: {e_rem}")
            except EnvironmentError as e_rem:
                print(f"Warning: Could not remove lock file {LOCKFILE}: {e_rem}")

    except Exception as e: print(f"Unexpected error removing lock file: {e}")

def _bytes2human(n):
    if not isinstance(n, (int, float)): return "N/A"
    symbols = ('B', 'KB', 'MB', 'GB', 'TB', 'PB'); prefix = {s: 1 << (i * 10) for i, s in enumerate(symbols)}
    for s in reversed(symbols):
        if n >= prefix[s]:
            try: value = float(n) / prefix[s]; return f'{value:.2f} {s}'
            except ValueError: return "Error"
    try: return f"{int(n)} B"
    except (ValueError, TypeError): return "N/A B"

def initialize_nvml():
    global nvml_initialized, nvml_handles, _pynvml_imported, pynvml
    with nvml_lock: # Acquire lock
        if nvml_initialized or _pynvml_imported: return nvml_initialized
        try:
            ...
            for i in range(gpu_count):
                ...
                gpu_name = pynvml.nvmlDeviceGetName(handle) # Already a string in this version
                # --- END OF CHANGE ---
            ...
            print("pynvml library not found. GPU monitoring disabled."); _pynvml_imported = True; return False # Set _pynvml_imported so we don't try again
        except Exception as e:
            err_str = f"{e}";
            # Ensure pynvml and necessary attributes exist before trying to use them
            if _pynvml_imported and pynvml and hasattr(pynvml, 'NVMLError') and isinstance(e, pynvml.NVMLError):
                 ...
                 try:
                     ...
                     err_str = f"{pynvml.nvmlErrorString(e)} (Code: {error_code})"
                      ...
                      print(f"Could not get NVML error string: {e_str}") # Log if getting string fails
            ...
            if _pynvml_imported and pynvml:
                try:
                    ...
                    if hasattr(pynvml, 'NVMLError') and isinstance(sd_e, pynvml.NVMLError):
                         try:
                              if sd_e.args and sd_e.args[0] == pynvml.NVML_ERROR_UNINITIALIZED:
                                   is_uninitialized_error = True
                         except Exception: pass
                    if not is_uninitialized_error:
                         print(f"Error during NVML shutdown after init failure: {sd_e}")

            nvml_initialized = False; nvml_handles = []; return False
...
def get_gpu_info(gpu_index):
    ...
    if not nvml_initialized or not _pynvml_imported or not pynvml or gpu_index >= len(nvml_handles):
        return {"temp": None, "util": None, "vram_used": 0, "vram_total": 0, "vram_percent": 0.0}

    handle_info = nvml_handles[gpu_index]; handle = handle_info["handle"]
    temp, util = None, None; vram_used, vram_total, vram_percent = 0, 0, 0.0

    try: temp = pynvml.nvmlDeviceGetTemperature(handle, pynvml.NVML_TEMPERATURE_GPU)
    except Exception: pass # Ignore errors (might happen temporarily)
    try: util = pynvml.nvmlDeviceGetUtilizationRates(handle).gpu
    except Exception: pass
    try:
        mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle); vram_used, vram_total = mem_info.used, mem_info.total
        vram_percent = (vram_used / vram_total) * 100 if vram_total else 0
    except Exception: pass
    return {"temp": temp, "util": util, "vram_used": vram_used, "vram_total": vram_total, "vram_percent": vram_percent }

def get_all_stats():
    global last_net_io, last_disk_io, last_time; current_time = time.time()
    ...
    return {
        ...
        "disk_read_speed": disk_read_speed, "disk_write_speed": disk_write_speed,
        "net_sent_speed": net_sent_speed, "net_recv_speed": net_recv_speed,
        "gpus": gpus_info
    }
...
class ResourceMonitorApp:
    def __init__(self, root):
        ...
        self.history = {
            ...
            "gpus_util": [collections.deque([0] * HISTORY_LENGTH, maxlen=HISTORY_LENGTH) for _ in range(num_gpus)],
            "gpus_vram": [collections.deque([0] * HISTORY_LENGTH, maxlen=HISTORY_LENGTH) for _ in range(num_gpus)]
        }
        ...
        available_themes = self.style.theme_names(); theme_to_use = 'clam' if 'clam' in available_themes else 'default'
        try: self.style.theme_use(theme_to_use)
        ...
        self.style.configure('BGUnit.TLabel', background=BG_COLOR, foreground=FG_COLOR, font=("Segoe UI", 8))
        self.style.configure('Error.TLabel', background=BG_COLOR, foreground="#ffcc00", font=("Segoe UI", 9))
        ...
        for section, accent, border, prog_bg in [
            ...
            ('IO', IO_ACCENT_COLOR, IO_FRAME_BORDER_COLOR, None) # IO has no progress bar
        ]:
            # Data Label Style (colored)
            self.style.configure(f'{section}.Data.TLabel', background=BG_COLOR, foreground=accent, font=("Segoe UI", 9, "bold"))
            # LabelFrame Style (tinted border)
            self.style.configure(f'{section}.TLabelframe', background=BG_COLOR, bordercolor=border, relief="groove", borderwidth=1)
            self.style.configure(f'{section}.TLabelframe.Label', background=BG_COLOR, foreground=FG_COLOR, font=("Segoe UI", 10, "bold"))
            # Progress Bar Style
            if prog_bg:
                self.style.configure(f'{section}.Horizontal.TProgressbar', troughcolor=prog_bg, background=accent, bordercolor=border, lightcolor=accent, darkcolor=accent)
        ...
        main_frame.grid_rowconfigure(next_row, weight=0) # IO section shouldn't expand vertically
    ...
    def _create_section_frame(self, parent, title, section_prefix):
        ...
        frame = ttk.LabelFrame(parent, text=title, style=f'{section_prefix}.TLabelframe', padding="5")
    ...
    def _add_info_row(self, frame, row_idx, label_text, section_prefix):
        ...
        value_label = ttk.Label(frame, text="N/A", style=f'{section_prefix}.Data.TLabel', anchor="e", width=15)
        value_label.grid(row=row_idx, column=1, sticky="ew", padx=5, pady=1) # sticky="ew" makes it expand
        # Unit label uses the default background style (or a specific Unit style if defined)
        unit_label = ttk.Label(frame, text="", style='BGUnit.TLabel', anchor="w") # anchor="w" keeps it left-aligned
    ...
    def _add_progress_bar(self, frame, row_idx, section_prefix):
        ...
        pb = ttk.Progressbar(frame, orient="horizontal", length=BAR_LENGTH, mode="determinate", style=f'{section_prefix}.Horizontal.TProgressbar')
    ...
    def _add_chart(self, frame, row_idx, data_key, gpu_index=None, label=True):
        ...
        canvas = tk.Canvas(frame, height=CHART_HEIGHT, width=BAR_LENGTH, bg=CANVAS_BG_COLOR, highlightthickness=1, highlightbackground=BORDER_COLOR)
        canvas.grid(row=row_idx + (1 if label else 0), column=0, columnspan=3, sticky="ew", padx=5, pady=(0,5))

        if gpu_index is None:
            if data_key == "cpu": self.cpu_chart_canvas = canvas
            elif data_key == "mem": self.mem_chart_canvas = canvas
        else:
    ...
    def _create_cpu_section(self, parent, row_index):
        ...
        self.cpu_util_val, self.cpu_util_unit = self._add_info_row(frame, next_row, "Utilization:", prefix)
        self.cpu_util_unit.configure(text="%")
        self.cpu_pb = self._add_progress_bar(frame, next_row + 1, prefix); next_row += 2

        # Create a sub-frame for the chart and temperature
        chart_frame = ttk.Frame(frame, style='TFrame')
        chart_frame.grid(row=next_row, column=0, columnspan=3, sticky="ew", padx=5, pady=(5,0))

        # Add chart canvas with reduced width
        self.cpu_chart_canvas = tk.Canvas(chart_frame, height=CHART_HEIGHT, width=BAR_LENGTH // 2, bg=CANVAS_BG_COLOR, highlightthickness=1, highlightbackground=BORDER_COLOR)
        self.cpu_chart_canvas.pack(side="left", fill="x", expand=True)

        # Add temperature label to the right of the chart
        self.cpu_temp_val = ttk.Label(chart_frame, text="N/A", style=f'{prefix}.Data.TLabel', font=("Segoe UI", 14, "bold"))
    ...
    def _create_mem_section(self, parent, row_index):
        ...
        frame.grid(row=row_index, column=0, sticky="ew", padx=5, pady=5)
        self.mem_usage_val, self.mem_usage_unit = self._add_info_row(frame, next_row, "Usage:", prefix)
        self.mem_percent_val, self.mem_percent_unit = self._add_info_row(frame, next_row + 1, "Percent:", prefix)
        self.mem_percent_unit.configure(text="%")
        self.mem_pb = self._add_progress_bar(frame, next_row + 2, prefix); next_row += 3
    ...
    def _create_gpu_sections(self, parent, start_row_index):
        ...
        if not nvml_initialized:
            ...
            no_gpu_label = ttk.Label(frame, text="NVIDIA GPU / NVML not detected or failed.", style='Error.TLabel')
        ...
        for i, gpu_handle_info in enumerate(nvml_handles):
            if i >= len(self.gpu_widgets): continue

            widgets = self.gpu_widgets[i]
            gpu_name = gpu_handle_info.get('name', f'GPU {i}')

            frame, next_row_in_frame = self._create_section_frame(parent, f"{gpu_name}", prefix)
            frame.grid(row=current_row, column=0, sticky="ew", padx=5, pady=5)
            widgets['frame'] = frame

            widgets['util_val'], widgets['util_unit'] = self._add_info_row(frame, next_row_in_frame, "Utilization:", prefix)
            widgets['util_unit'].configure(text="%")
            widgets['util_pb'] = self._add_progress_bar(frame, next_row_in_frame + 1, prefix); next_row_in_frame += 2

            # Create a sub-frame for the utilization chart and temperature
            util_chart_frame = ttk.Frame(frame, style='TFrame')
            util_chart_frame.grid(row=next_row_in_frame, column=0, columnspan=3, sticky="ew", padx=5, pady=(5,0))

            # Add utilization chart canvas with reduced width
            util_canvas = tk.Canvas(util_chart_frame, height=CHART_HEIGHT, width=BAR_LENGTH // 2, bg=CANVAS_BG_COLOR, highlightthickness=1, highlightbackground=BORDER_COLOR)
            ...
            widgets['temp_val'] = ttk.Label(util_chart_frame, text="N/A", style=f'{prefix}.Data.TLabel', font=("Segoe UI", 14, "bold"))
            widgets['temp_val'].pack(side="right", padx=(5,0))

            next_row_in_frame += 1

            widgets['vram_val'], widgets['vram_unit'] = self._add_info_row(frame, next_row_in_frame, "VRAM Usage:", prefix)
            widgets['vram_perc_val'], widgets['vram_perc_unit'] = self._add_info_row(frame, next_row_in_frame + 1, "VRAM Percent:", prefix)
            widgets['vram_perc_unit'].configure(text="%")
            widgets['vram_pb'] = self._add_progress_bar(frame, next_row_in_frame + 2, prefix); next_row_in_frame += 3

            # Create a sub-frame for the VRAM chart
            vram_chart_frame = ttk.Frame(frame, style='TFrame')
            vram_chart_frame.grid(row=next_row_in_frame, column=0, columnspan=3, sticky="ew", padx=5, pady=(5,0))

            # Add VRAM chart canvas with reduced width
            vram_canvas = tk.Canvas(vram_chart_frame, height=CHART_HEIGHT, width=BAR_LENGTH // 2, bg=CANVAS_BG_COLOR, highlightthickness=1, highlightbackground=BORDER_COLOR)
    ...
    def _create_disk_section(self, parent_container):
        ...
        frame, next_row = self._create_section_frame(parent_container, "Disk I/O", prefix)
        ...
        self.disk_read_val, self.disk_read_unit = self._add_info_row(frame, next_row, "Read:", prefix)
        self.disk_write_val, self.disk_write_unit = self._add_info_row(frame, next_row + 1, "Write:", prefix)

    def _create_network_section(self, parent_container):
        prefix = "IO" # Use IO prefix for neutral styling
        frame, next_row = self._create_section_frame(parent_container, "Network I/O", prefix)
        ...
        self.net_recv_val, self.net_recv_unit = self._add_info_row(frame, next_row, "Down:", prefix)
        self.net_sent_val, self.net_sent_unit = self._add_info_row(frame, next_row + 1, "Up:", prefix)


    # --- Chart Update Method ---
    def _update_chart(self, canvas, data_deque, line_color, max_value=100, overlay_text=None):
        ...
        if not width or not height or width < 2 or height < 2 or len(data_deque) < 2 or max_value <= 0:
            return

        y_range = height - 2
        for perc in [0.25, 0.5, 0.75]:
            y_line = 1 + y_range - (perc * y_range)
            canvas.create_line(1, y_line, width - 1, y_line, fill=CANVAS_GRID_COLOR, dash=(2, 4))

        points = []
        x_step = (width - 2) / (HISTORY_LENGTH - 1) if HISTORY_LENGTH > 1 else (width - 2)

        for i, value in enumerate(data_deque):
            clamped_value = max(0.0, min(float(value if value is not None else 0.0), float(max_value)))
        ...
        if len(points) >= 4:
            try:
                ...
                if overlay_text is not None:
                    canvas.create_text(width // 2, height // 2, text=overlay_text, fill=line_color, font=("Segoe UI", 10))
            except tk.TclError:
                pass
            except Exception:
                pass
    ...
    def update_gui(self):
        ...
        try:
            ...
            for i in range(min(num_gpus_detected, len(self.history["gpus_util"]))):
                 ...
                 self.history["gpus_util"][i].append(gpu_info.get("util", 0) if gpu_info.get("util") is not None else 0)
            ...
            # CPU
            try: self.cpu_util_val.configure(text=f"{stats.get('cpu_percent', 0):.1f}")
            except tk.TclError: pass
            try: self.cpu_pb['value'] = stats.get('cpu_percent', 0)
            except tk.TclError: pass
            try: cpu_temp = stats.get('cpu_temp'); self.cpu_temp_val.configure(text=f"{cpu_temp:.0f}" if cpu_temp is not None else "N/A")
            except tk.TclError: pass

            # Memory
            try: self.mem_usage_val.configure(text=f"{_bytes2human(stats.get('mem_used', 0))} / {_bytes2human(stats.get('mem_total', 0))}")
            except tk.TclError: pass
            try: self.mem_percent_val.configure(text=f"{stats.get('mem_percent', 0):.1f}")
            except tk.TclError: pass
            try: self.mem_pb['value'] = stats.get('mem_percent', 0)
            except tk.TclError: pass

            # Disk IO
            try: self.disk_read_val.configure(text=f"{_bytes2human(stats.get('disk_read_speed', 0))}"); self.disk_read_unit.configure(text="/s")
            except tk.TclError: pass
            try: self.disk_write_val.configure(text=f"{_bytes2human(stats.get('disk_write_speed', 0))}"); self.disk_write_unit.configure(text="/s")
            except tk.TclError: pass

            # Network IO
            try: self.net_recv_val.configure(text=f"{_bytes2human(stats.get('net_recv_speed', 0))}"); self.net_recv_unit.configure(text="/s")
            except tk.TclError: pass
            try: self.net_sent_val.configure(text=f"{_bytes2human(stats.get('net_sent_speed', 0))}"); self.net_sent_unit.configure(text="/s")
            except tk.TclError: pass
            ...
            for i, gpu_info in enumerate(stats.get("gpus", [])):
                if i < len(self.gpu_widgets):
                    ...
                    try:
                        widgets['util_val'].configure(text=f"{util:.0f}" if util is not None else "N/A")
                        widgets['util_pb']['value'] = util if util is not None else 0
                        if self.gpu_util_chart_canvases[i]:
                            self._update_chart(self.gpu_util_chart_canvases[i], self.history["gpus_util"][i], GPU_ACCENT_COLOR, overlay_text=f"{temp:.0f}°C" if temp is not None else "N/A")
                    except tk.TclError:
                        pass
                    try:
                        widgets['temp_val'].configure(text=f"{temp:.0f}°C" if temp is not None else "N/A")
                    except tk.TclError:
                        pass
                    try:
                        widgets['vram_val'].configure(text=f"{_bytes2human(vram_used)} / {_bytes2human(vram_total)}")
                        widgets['vram_perc_val'].configure(text=f"{vram_perc:.1f}")
                        widgets['vram_pb']['value'] = vram_perc
                        if self.gpu_vram_chart_canvases[i]:
                            self._update_chart(self.gpu_vram_chart_canvases[i], self.history["gpus_vram"][i], GPU_ACCENT_COLOR)
                    except tk.TclError:
                        pass
            ...
            try:
                if hasattr(self, 'cpu_chart_canvas'): self._update_chart(self.cpu_chart_canvas, self.history["cpu"], CPU_ACCENT_COLOR)
            except Exception as e: print(f"Error updating CPU chart: {e}")
            try:
                if hasattr(self, 'mem_chart_canvas'): self._update_chart(self.mem_chart_canvas, self.history["mem"], RAM_ACCENT_COLOR)
            except Exception as e: print(f"Error updating Mem chart: {e}")

            # Use the correct list of canvases
            try:
                for i, canvas in enumerate(self.gpu_util_chart_canvases):
                     if canvas and i < len(self.history["gpus_util"]): self._update_chart(canvas, self.history["gpus_util"][i], GPU_ACCENT_COLOR)
            except Exception as e: print(f"Error updating GPU Util chart: {e}")
            try:
                 for i, canvas in enumerate(self.gpu_vram_chart_canvases):
                     if canvas and i < len(self.history["gpus_vram"]): self._update_chart(canvas, self.history["gpus_vram"][i], GPU_ACCENT_COLOR)
            except Exception as e: print(f"Error updating GPU VRAM chart index: {e}")
            ...
            try:
                if self.root and self.root.winfo_exists():
                    ...
                    self._after_id = self.root.after(UPDATE_INTERVAL_MS, self.update_gui)
                # else:
    ...
    def on_closing(self):
        ...
        try:
             ...
             if self._after_id:
                ...
                try:
                     self.root.after_cancel(self._after_id)
                except tk.TclError: pass # Ignore error if root is already gone
                self._after_id = None
            ...
            print(f"TclError during on_closing: {e}") # Should not happen often if winfo_exists check works
        except Exception as e:
...
def launch_monitor_gui():
    global _monitor_active_flag; _monitor_active_flag = True
    ...
    try:
        ...
        app = ResourceMonitorApp(root) # Pass the root window

        # Make window appear after setup
        root.deiconify()
        root.lift()

        # Apply the always-on-top setting based on constant
        if ALWAYS_ON_TOP:
            root.attributes('-topmost', True)  # Always keep on top of other windows
        else:
            # Original behavior: only briefly on top, then normal
            root.attributes('-topmost', True)  # Keep it on top initially
            root.after_idle(lambda: root.attributes('-topmost', False))

        # print("Starting Tkinter mainloop...");
        root.mainloop() # This blocks the *GUI thread* until the window is closed
        ...
        # print("GUI thread finally block executing...")
        # --- Cleanup within the thread ---
        shutdown_nvml() # Shutdown NVML in the same thread it was initialized
        remove_lockfile() # Remove lock file as this instance is closing
        _monitor_active_flag = False # Mark as inactive for this process
        # print("GUI thread cleanup complete.")
        # Restore original signal handlers if they were changed by this thread
...
def resource_monitor():
    ...
    if is_monitor_running():
        print("\033[93mResource Monitor GUI appears to be already running (found lock file for another process).\033[0m")
        return
    ...
    try:
        ...
        # print(f"Resource monitor started in detached process")
        return True
