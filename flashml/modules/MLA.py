...
class MLA(nn.Module):
    ...
    '''
    def __init__(self, embedding_dim: int, heads_num: int, max_seq_len: int = 4096, is_causal: bool = False,  dropout=0.0):
        super().__init__()
        ...
        assert kv_compression_dim < self.dh * self.nh, "latent_dim must be way smaller than the total number of heads"
        assert q_compression_dim < self.dh * self.nh, "latent_dim must be way smaller than the total number of heads"
        ...
        self.W_UK = nn.Linear(self.dc, self.dh * self.nh, bias=False) # W_UK & W_UV in R_dhnh*dc
        self.W_UV = nn.Linear(self.dc, self.dh * self.nh, bias=False)
        self.W_KR = nn.Linear(self.d, self.dhR, bias=False) # W_KR in R_dRh x d

        self.W_DQ = nn.Linear(embedding_dim, self.dc_prime, bias=False) # W_DQ in R_dc_prime x d
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, L, D = x.shape
        assert D == self.d, f"Input embedding dim {D} does not match the expected dim {self.d}"
        ...
        k_R = self.rope(self.W_KR(x)) # (B, L, nh*dh) # TO BE CACHED-----------------------------------
