...
class pRoPE(nn.Module):
    def __init__(self, head_dim: int, max_wavelength: int = 10_000, rope_percentage: float = 1.0):
        super().__init__()
        if head_dim % 2 != 0:
            raise ValueError(f"head_dim must be an even number, but got {head_dim}")
        if not (0.0 <= rope_percentage <= 1.0):
             raise ValueError(f"rope_percentage must be between 0.0 and 1.0, but got {rope_percentage}")
        ...
        if rope_angles > 0:
            fraction = 2. * torch.arange(0, rope_angles) / self.head_dim
    ...
    def forward(self, x: torch.Tensor, positions: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if head_dim != self.head_dim:
             raise ValueError(f"Input tensor's head_dim (last dim={head_dim}) does not match "
        ...
        if positions is None:
            ...
            if positions.shape != (B, L):
                raise ValueError(f"Provided positions shape {positions.shape} does not match "
